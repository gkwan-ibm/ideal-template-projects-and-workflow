...
(To be confirmed) The [hotspot=httpEndpoint file=1]`httpEndpoint` ports are defined by the [hotspot=httpPorts file=1]`defaultValue` of the `default.http.port` and `default.http.port` variables. They can be overridden through the [hotspot=httpPortProperties file=0]`pom.xml`.

(To be confirmed) The webApplication [hotspot=webApplication file=1]`location` should be [hotspot=artifactId file=0]`${artifactId}.war`. The [hotspot=httpEndpoint file=1]`contextRoot` is defined by the [hotspot=appContextRoot file=1]`defaultValue` of the `app.context.root` variable. It can be overridden through the [hotspot=contextRootProperty file=0]`pom.xml`.

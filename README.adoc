...
(To be confirmed) The [hotspot=httpEndpoint file=1]`httpEndpoint` ports are defined by the [hotspot=httpPorts file=1]`defaultValue` of the `default.http.port` and `default.http.port` variables. They can be overridden through the [hotspot=libertyProperties file=0]`pom.xml`.
